...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_changes') {
    ...
    if (!empty($selected_rows)) {
        try {
            ...
            $sql_update = "UPDATE tabela_precos_vistorias SET tipo_servico = :tipo_servico, modalidade = :modalidade, regiao = :regiao, valor = :valor, ativo = :ativo WHERE id = :id";
            $stmt_update = $pdo->prepare($sql_update);
            
            foreach ($selected_rows as $id) {
                $stmt_update->execute([
                    'tipo_servico' => $_POST['tipo_servico'][$id] ?? '',
                    'modalidade' => $_POST['modalidade'][$id] ?? '',
                    'regiao' => $_POST['regiao'][$id] ?? '',
                    'valor' => $_POST['valor'][$id] ?? 0,
                    'ativo' => $_POST['ativo'][$id] ?? 'false',
                    'id' => $id
                ]);
            }
...
$servico_filter = isset($_GET['tipo_servico']) ? trim($_GET['tipo_servico']) : '';
$regiao_filter = isset($_GET['regiao']) ? trim($_GET['regiao']) : '';

$where = ["1=1"];
$params = [];

if ($servico_filter !== '') {
    $where[] = "tipo_servico ILIKE :tipo_servico";
    $params['tipo_servico'] = "%$servico_filter%";
}

if ($regiao_filter !== '') {
    $where[] = "regiao ILIKE :regiao";
    $params['regiao'] = "%$regiao_filter%";
}

$where_clause = implode(' AND ', $where);

try {
    $count_sql = "SELECT COUNT(*) FROM tabela_precos_vistorias WHERE $where_clause";
...
if (!isset($error_msg)) {
    try {
        $sql = "SELECT * FROM tabela_precos_vistorias WHERE $where_clause ORDER BY id DESC LIMIT $limit OFFSET $offset";
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>ID</th>
                    <th>Tipo Serviço (Editável)</th>
                    <th>Modalidade (Editável)</th>
                    <th>Região (Editável)</th>
                ...
                <?php else: ?>
                    <?php foreach ($data as $row): ?>
                        <tr id="row-<?php echo $row['id']; ?>">
                            <td><input type="checkbox" name="selected_rows[]" value="<?php echo $row['id']; ?>" class="row-checkbox"></td>
                            <td><?php echo htmlspecialchars($row['id'] ?? ''); ?></td>
                            <td><input type="text" name="tipo_servico[<?php echo $row['id']; ?>]" value="<?php echo htmlspecialchars($row['tipo_servico'] ?? ''); ?>" class="edit-input data-input"></td>
                            <td><input type="text" name="modalidade[<?php echo $row['id']; ?>]" value="<?php echo htmlspecialchars($row['modalidade'] ?? ''); ?>" class="edit-input data-input"></td>
                            <td><input type="text" name="regiao[<?php echo $row['id']; ?>]" value="<?php echo htmlspecialchars($row['regiao'] ?? ''); ?>" class="edit-input data-input"></td>
                            <td><input type="number" step="0.01" name="valor[<?php echo $row['id']; ?>]" value="<?php echo htmlspecialchars($row['valor'] ?? '0.00'); ?>" class="edit-input data-input"></td>
                            <td>
                                <select name="ativo[<?php echo $row['id']; ?>]" class="edit-input data-input">
                                    <option value="true" <?php echo ($row['ativo'] ?? '') === 'true' ? 'selected' : ''; ?>>SIM</option>
                                    <option value="false" <?php echo ($row['ativo'] ?? '') === 'false' ? 'selected' : ''; ?>>NÃO</option>
                                </select>
                            </td>
                        </tr>
                    <?php endforeach; ?>
...
<head>
    ...
    <title>Tabela de Preços - Vistorias</title>
        ...
        <form id="filterForm" method="GET">
            <div class="filter-grid">
                <div class="filter-group">
                    ...
                    <input type="text" name="tipo_servico" id="tipo_servico" value="<?php echo htmlspecialchars($servico_filter); ?>" placeholder="Buscar serviço...">
                </div>
                <div class="filter-group">
                    <label for="regiao">Região</label>
                    <input type="text" name="regiao" id="regiao" value="<?php echo htmlspecialchars($regiao_filter); ?>" placeholder="Buscar região...">
                </div>
                <div class="btn-group">
                    <a href="tabela_precos.php" class="btn btn-clear">Limpar</a>
...
<script>
    ...
    function loadData(page = 1) {
        ...
        fetch('tabela_precos.php?' + params.toString())
    ...
    // Debounce na busca
    ['tipo_servico', 'regiao'].forEach(id => {
